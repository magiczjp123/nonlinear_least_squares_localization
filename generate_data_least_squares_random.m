...
w_random = 2 * rand(numP, 1) - 1;
data(1:numP,5) = w_random(1:numP,1);
...
for i=1:landmarks;
    Feature(i,1) = (max(Pose(:,1))-min(Pose(:,1)))*rand(1,1) + min(Pose(:,1));
    Feature(i,2) = (max(Pose(:,2))-min(Pose(:,2)))*rand(1,1) + min(Pose(:,2));
end;
% 
% hold on;
...
for i=1:numP+1;
    for k=1:landmarks;
        ...
        BOF(ind,1) = atan((Feature(k,2)-Pose(i,2))/(Feature(k,1)-Pose(i,1))) - Pose(i,3)+N_bearing_obs(ind,1);
        if BOF(ind,1) > 2*pi
...
file = strcat(Direction,'localrandommap_test');
